...
class NeuralIBM1Model:
  ...
  def _build_model(self):
    ...
    pa_x = tf.expand_dims(pa_x, -1)  # Shape: [B, 1, M, 1]
    ...
    pa_x = tf.tile(pa_x, [1, longest_y, 1 , longest_y])  # [B, N, M, N] 
    print(pa_x.get_shape())
    ...
    # Shape: [B, 1, M, emb_dim] where B is batch size, M is (longest) source sentence length.
    # xy_embedded = tf.expand_dims(x_embedded, 1)
    # xy_embedded = tf.tile(xy_embedded, [1, 1,longest_y, 1])  # [B, M , N, emb_dim]

    y_shifted = tf.slice(y_embedded, [0,0,0], [-1,-1,-1])
    #y_shifted = tf.cast(y_shifted, tf.int32)
    zeros = tf.zeros([1,1,self.emb_dim])
    print(zeros.get_shape())
    print(y_shifted.get_shape())
    y_shifted = tf.concat([zeros,y_shifted],1)
    print(y_shifted.get_shape())
    y_shifted = tf.expand_dims(y_shifted,0)
    print(y_shifted.get_shape())

    # Wa, Wb, Wr, Wt, ba, bb, br, bt
    
    ha = tf.tanh(y_shifted)
    alpha = tf.exp(tf.matmul(Wa, ha) + ba)

    hb = tf.tanh(y_shifted)
    beta = tf.exp(tf.matmul(Wb, hb) + bb)

    s = tf.kuma(alpha, beta)
    s_1 = tf.subtract(1.0, s)

    embedded_gate = tf.matmul(x_embedded, s)
    y_shifted_gate = tf.matmul(y_shifted, s_1)
    ref = tf.add([embedded_gate, y_shifted_gate])

    r = tf.tanh(tf.matmul(Wr, ref) + br)

    t_theta = tf.softmax(tf.matmul(Wt, r) + bt)
    ...
    #  
    py_x = tf.matmul(pa_x, py_xa)  # Shape: [B, N, N, Vy]
